<?php
/**
 * HHPessoasWindow Registration
 * @author  <your name here>
 */
class HHPessoasWindow extends TWindow
{
    protected $form; // form
    ...
    {
        parent::__construct();
        parent::setTitle( AdiantiCoreTranslator::translate('Person') . ': ' .
        ...
        // creates the form
        $this->form = new BootstrapFormBuilder('form_PessoasWindow');
        ...
        $tipo_residencia_id->setSize('100%');
        ...
        $estadonacto->setEditable(FALSE);
        $paisnacto->enableSearch();
        $paisnacto->setEditable(FALSE);
        ...
        $this->form->addFields( [ new TLabel(_t('Commercial Board')) ], [ $junta_comercial ], [  ], [  ] );
    ...
    public function onSave( $param )
    {
        try
        {
            ...
            if( !empty($data->grupos_pessoas) )
            {
                foreach( $param['grupos_pessoas'] as $grupo )
                {
            ...
            TForm::sendData('form_PessoasWindow', $data);
    ...
    public static function onChangeTipo($param)
    {
        ...
        TScript::create('$($(\'form[name=form_PessoasWindow]\').find(".nav-tabs li")[1]).hide();');
        TScript::create('$($(\'form[name=form_PessoasWindow]\').find(".nav-tabs li")[2]).hide();');
            
        if ($value == 'F')
        {
            TScript::create('$($(\'form[name=form_PessoasWindow]\').find(".nav-tabs li")[1]).show();');
            TScript::create('$($(\'form[name=form_PessoasWindow]\').find(".nav-tabs li")[2]).hide();');
                
            TQuickForm::showField('form_PessoasWindow', 'tipo_residencia_id');
        }
            
        if ($value == 'J')
        {
            TScript::create('$($(\'form[name=form_PessoasWindow]\').find(".nav-tabs li")[1]).hide();');
            TScript::create('$($(\'form[name=form_PessoasWindow]\').find(".nav-tabs li")[2]).show();');
                
            TQuickForm::hideField('form_PessoasWindow', 'tipo_residencia_id');
        }
    }
        ...
        try
        {
            ...
            if ($pessoas)
            {
                foreach ($pessoas as $pessoa)
                {
                    ...
                    TForm::sendData('form_PessoasWindow', $object);
                }
            }
        ...
        try
        {
            ...
            if ($cidades)
            {
                foreach ($cidades as $cidade)
                {
                    ...
                    TForm::sendData('form_PessoasWindow', $obj);
                }
            }
    ...
    public static function onChangeNacionalidade($param)
    {
        ...
        if ($value == 'B')
        {
            ...
            TDBCombo::disableField('form_PessoasWindow', 'paisnacto');
        }
    }
    ...
    public static function onChangeFormaPagamento($param)
    {
        try
        {
            ...
            {
                TDBUniqueSearch::enableField('form_PessoasWindow', 'banco_id');
                TDBCombo::enableField('form_PessoasWindow', 'tipo_conta_id');
                TEntry::enableField('form_PessoasWindow', 'agencia');
                TEntry::enableField('form_PessoasWindow', 'conta');
                TEntry::enableField('form_PessoasWindow', 'operacao');
            }
            else
            {
                TDBUniqueSearch::disableField('form_PessoasWindow', 'banco_id');
                TDBCombo::disableField('form_PessoasWindow', 'tipo_conta_id');
                TEntry::disableField('form_PessoasWindow', 'agencia');
                TEntry::disableField('form_PessoasWindow', 'conta');
                TEntry::disableField('form_PessoasWindow', 'operacao');
            }
    ...
    {
        $errors = array();
        $fields = array(_t('Type') => $param['tipo'], 'CPF/CNPJ' => $param['cpfcnpj'], _t('Name') => $param['nome']);
