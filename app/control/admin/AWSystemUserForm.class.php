 ...
 */
class AWSystemUserForm extends TPage
{
    protected $form; // form
    ...
    public function __construct()
    {
        ...
        foreach ($user->getSystemUserGroups() as $groups)
        {
            foreach ($groups->getSystemPrograms() as $prog)
            {
                $programs[] = $prog->id;
            }
        }
        
        foreach ($user->getSystemUserPrograms() as $prog)
        {
            $programs[] = $prog->id;
        }
        ...
        $criteria_groups->add(new TFilter('id', 'in', TSession::getValue('usergroupids')));

        $subform->appendPage( _t('Groups') );
        $this->group_list = new TDBCheckList('group_list', 'permission', 'SystemGroup', 'id', 'name', 'name', $criteria_groups);
        ...
        $criteria_units->add(new TFilter('id', 'in', TSession::getValue('userunitids')));

        $subform->appendPage( _t('Roles') );
        $this->role_list = new TDBCheckList('role_list', 'permission', 'SystemRole', 'id', 'name', 'name', $criteria_units);
        ...
        $this->program_list->addColumn('id',    _t('Id'),    'center',  '10%');
        ...
        $criteria_programs = new TCriteria;
        $criteria_programs->add(new TFilter('id', 'in', $programs));

        $repository = new TRepository('SystemProgram');
        $objects = $repository->load($criteria_programs);

        foreach ($objects as $object)
        {
            $this->program_list->addItem( $object );
        }
    ...
    public function onSave($param)
    {
        ...
        try
        {
            ...
            $pos_action = new TAction(['AWSystemUserList', 'onReload']);
