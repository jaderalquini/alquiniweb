<?php
/**
 * FeriadosList Listing
 * @author  Jáder Alexandre Alquini
 */
class FeriadosList extends TStandardList
{
    ...
    public function __construct()
    {
        ...
        parent::setActiveRecord('Feriados');   // defines the active record
        parent::setDefaultOrder('id', 'asc');         // defines the default order
        parent::addFilterField('id', '=', 'id'); // filterField, operator, formField
        parent::addFilterField('dt_feriado', '=', 'dt_feriado'); // filterField, operator, formField
        ...
        // creates the form
        $this->form = new BootstrapFormBuilder('form_search_Feriados');
        ...
        $this->form->addFields( [new TLabel(_t('Date'))], [$dtferiado] );
        ...
        $this->form->setData( TSession::getValue('Feriados_filter_data') );
        ...
        $column_dtferiado = new TDataGridColumn('dtferiado', _t('Date'), 'center', 50);
        $column_descricao = new TDataGridColumn('descricao', _t('Description'), 'left');

        $this->datagrid->addColumn($column_id);
        $this->datagrid->addColumn($column_dtferiado);
        $this->datagrid->addColumn($column_descricao);

        $column_id->setTransformer([$this, 'formatRow'] );

        $column_dtferiado->setTransformer( function($value) {
            if ($value)
            {
                try
                {
        ...
        $order_dtferiado = new TAction(array($this, 'onReload'));
        $order_dtferiado->setParameter('order', 'dt_feriado');
        $column_dtferiado->setAction($order_dtferiado);
        ...
        // create EDIT action
        $action_edit = new TDataGridAction(array('FeriadosForm', 'onEdit'), ['register_state' => 'false'] );
        ...
        $panel->addHeaderActionLink(_t('New'), new TAction(['FeriadosForm', 'onEdit'], ['register_state' => 'false']), 'fa:plus green');
